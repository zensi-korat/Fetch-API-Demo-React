...
function SidebarGroup({
  ...
  return (
    <SidebarGroupContext.Provider value={{ open, setOpen, collapsible }}>
      <div
        data-slot="sidebar-group"
        data-sidebar="group"
        data-state={open ? "open" : "closed"}
        className={cn("group flex w-full min-w-0 flex-col p-2", className)}
        {...props}
      >
        {children}
      </div>
...
function SidebarGroupLabel({
  ...
  const isCollapsible = context?.collapsible;
  ...
  return (
    <Comp
      ...
      aria-expanded={isCollapsible ? context?.open : undefined}
      onClick={() => {
        if (isCollapsible) {
      ...
      className={cn(
        "text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center justify-between rounded-md px-2 text-xs font-medium outline-hidden transition-all duration-200 ease-linear focus-visible:ring-2",
